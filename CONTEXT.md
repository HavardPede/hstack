...
- **Agent Skill** — one directory under `plugins/hstack/skills/` with portable `name` and `description` frontmatter. Codex, Prime Agent, opencode, and Gemini CLI discover this shared tree natively. Runtime-specific keys may affect visibility only where that runtime understands them.
- **Model policy** — the mapping from roles to model slugs. Defaults live in `plugins/hstack/models.json` and are stamped into each skill's Models section; the user's override sheet (`~/.claude/hstack-models.md`, written by `setup-hstack`) is the layer that adapts them at runtime.
- **Role** — a named unit of delegated work with its own model choice (`arena runners`, `how critics`, `swarm workers`). The role vocabulary lives in `models.json` and surfaces in `setup-hstack`'s sheet.
...
- **Prompt stub** (or trampoline) — a file under `.codex-plugin/prompts/` whose body invokes its skill. Codex-only, generated from the skill's README slash-command row; every other supported runtime discovers the shared Agent Skill directly.
- **Menu description** — the one-liner in a public skill's README slash-command row. Renders as the Codex slash-menu text; the skill's long `description:` frontmatter stays the trigger-matching prose.
